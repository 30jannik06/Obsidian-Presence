	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addButton((btn) => {
				...
				btn.setButtonText(paused ? "Resume" : "Pause")
					.setCta()
					.onClick(() => {
		...
		if (this.plugin.settings.showVaultName) {
			new Setting(containerEl)
				...
				.setDesc(
					"Override the vault name shown in Discord. Leave empty to use the real vault name."
				)
				.addText((text) =>
					text
		...
		new Setting(containerEl).setName("Excluded patterns").addTextArea((area) =>
			area
		...
		// ── Idle detection ─────────────────────────────────────────────────────
		new Setting(containerEl).setName("Idle detection").setHeading();
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Up to 2 buttons shown on your Discord profile. Leave label or URL empty to disable a button."
			)
			.setHeading();
